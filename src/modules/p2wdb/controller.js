...
const WriteEntry = require('../../use-cases/write-entry')
...
class PTWDBController {
  constructor (localConfig) {
    ...
    this.writeEntry = new WriteEntry({
      p2wdb: () => {},
      localDb: () => {}
    })
  }
  ...
  async writeToDb (ctx) {
    try {
      ...
      // const value = {
      //   messages: ctx.request.body.message,
      //   signature: ctx.request.body.signature,
      //   data: ctx.request.body.data
      // }
      //
      // const rawData = {
      //   key: ctx.request.body.txid,
      //   value
      // }

      // const hash = await this.writeEntry.addEntry(rawData)
